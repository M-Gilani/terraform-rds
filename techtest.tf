...
resource "aws_vpc" "dbase_vpc" { # creating vpc to place services in.
...
resource "aws_db_subnet_group" "example_db_subnet_group" {
  name       = "example-db-subnet-group"  # just a place holder name.
  subnet_ids = [
...
resource "aws_db_instance" "ecomrds" {
  ...
  storage_encrypted      = true # so it is even more secure as any data will be encrypted.
  backup_retention_period = 7   # should automatically create backups.
}
